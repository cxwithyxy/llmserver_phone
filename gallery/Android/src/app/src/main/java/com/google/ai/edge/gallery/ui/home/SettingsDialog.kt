...
import com.google.ai.edge.gallery.webservice.LlmWebServerService
...
fun SettingsDialog(
  ...
  var selectedWebServiceModel by remember {
    mutableStateOf(
      when {
        storedModelName.isNotBlank() -> storedModelName
        availableWebModels.isNotEmpty() -> availableWebModels.first().name
        else -> ""
      },
    )
  }
    ...
    Card(
      ...
      Column(
        ...
        Column(
          ...
          Column(modifier = Modifier.fillMaxWidth().semantics(mergeDescendants = true) {}) {
            val hasModels = availableWebModels.isNotEmpty()
            Row(verticalAlignment = Alignment.CenterVertically) {
              Column(modifier = Modifier.weight(1f)) {
                Text(
                  stringResource(R.string.settings_web_service_title),
                  style = MaterialTheme.typography.titleSmall.copy(fontWeight = FontWeight.Medium),
                )
                Text(
                  stringResource(R.string.settings_web_service_description),
              ...
              Switch(
                ...
                onCheckedChange = { enabled ->
                  if (enabled && !hasModels) {
                    webServiceEnabled = false
                    return@Switch
                  }
                  webServiceEnabled = enabled
                  if (enabled && selectedWebServiceModel.isBlank() && hasModels) {
                    selectedWebServiceModel = availableWebModels.first().name
                  }
                  modelManagerViewModel.setWebServiceEnabled(enabled)
                  if (enabled) {
                    if (selectedWebServiceModel.isNotBlank()) {
                      modelManagerViewModel.setWebServiceModelName(selectedWebServiceModel)
                    }
                    LlmWebServerService.start(context)
                  } else {
                    LlmWebServerService.stop(context)
                  }
                },
                enabled = hasModels || webServiceEnabled,
                colors = SwitchDefaults.colors(),
              )
            }
            if (webServiceEnabled) {
              Text(
                stringResource(R.string.settings_web_service_model_label),
                style = MaterialTheme.typography.bodySmall.copy(fontWeight = FontWeight.Medium),
                modifier = Modifier.padding(top = 12.dp, bottom = 4.dp),
              )
              if (availableWebModels.isEmpty()) {
                Text(
                  stringResource(R.string.settings_web_service_no_model),
                  style = MaterialTheme.typography.bodySmall,
                ...
                Box {
                  OutlinedButton(
                    ...
                    val label =
                      if (selectedWebServiceModel.isBlank()) {
                        stringResource(R.string.settings_web_service_model_label)
                      } else {
                        selectedWebServiceModel
                      }
                    Text(label)
                  }
                  DropdownMenu(expanded = modelMenuExpanded, onDismissRequest = { modelMenuExpanded = false }) {
                    availableWebModels.forEach { model ->
                      DropdownMenuItem(
                        text = { Text(model.name) },
                        onClick = {
                          selectedWebServiceModel = model.name
                          modelManagerViewModel.setWebServiceModelName(model.name)
